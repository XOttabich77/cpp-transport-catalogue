...
router::TransportRouter::TransportRouter(const transport::TransportCatalogue& db,
    ...
    db_(db)
{
    using namespace std::literals;
    routing_setting_.SetWaitingTime(wait_time).SetBusSpeed(speed);
    route_map__ = graph::DirectedWeightedGraph<Time>(edges,il);
    MakeMapRouter(false);
}
...
void router::TransportRouter::MakeMapRouter(bool flag)
{       
    MakeWaitEdge(flag);
    MakeBusEdge(flag);
...
void router::TransportRouter::MakeWaitEdge(bool flag)
{
    graph::VertexId count = 0;
    for (auto& stop : db_.GetAllStops())
    {
        if (flag) {
            edge_id_ = route_map__.AddEdge({ count ,count + 1,routing_setting_.bus_wait_time });
        }

        stop_id_[stop.name].wait = count++;
        stop_id_[stop.name].go = count++;
        wait_edge_[edge_id_++] = { stop.name, routing_setting_.bus_wait_time };
    }
}

void router::TransportRouter::MakeBusEdge(bool flag)
{
    for (const auto& bus : db_.GetAllBus()) {
        for (auto it_from = bus.stops.begin(); it_from != prev(bus.stops.end()); ++it_from) {
            ...
            for (auto it_to = next(it_from); it_to != bus.stops.end(); ++it_to) {
                ...
                ++stop_count;
                if (flag) {
                    edge_id_ = route_map__.AddEdge({ id_from,id_to,time_to_stop });
                }
                bus_edge_[edge_id_++] = { bus.name, time_to_stop ,stop_count };
            }
        }
    }
}
